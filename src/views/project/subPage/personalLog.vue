...
<script>
  import {getPersonalList,getPersonalDetail,add,update,remove} from "@/api/project/personalLog";
    ...
    data() {
      ...
      const checkDate = (rule, value, callback) =>  {
        setTimeout(() => {
          ...
          if (value !== undefined && value !== '') {
            // 获取开始日期
            const { reportStartDate } = this.form
            // 判断是否已经选择了开始日期，没有则提示请先选择开始日期
            if (reportStartDate !== undefined && reportStartDate !== '') {
              const begin = new Date(reportStartDate);
    ...
    methods:{
      ...
      beforeOpenDialog(done,type) {
        ...
        if (["view"].includes(type)) {
          ...
          getPersonalDetail(this.form.id).then(res => {
      ...
      onLoad(page, params = {}) {
        ...
        getPersonalList(page.currentPage, page.pageSize, Object.assign(params, this.query)).then(res => {
